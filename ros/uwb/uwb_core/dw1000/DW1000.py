...
import spidev
from .linux_io import GPIO   # libgpiod-v2 shim (RPi.GPIO is broken on Pi5/CM5)
...
class DW1000(object):
    ...
    # True when the last startTransmit() armed WAIT4RESP, so the TXFRS handler
    # must NOT re-arm RX in software (the hardware already did, with no host
    # round-trip). See startTransmit() / handleInterrupt() for the rationale.
    _w4r_armed = False
    ...
    def __init__(self, **kwargs):
        ...
        # Serializes every SPI transaction + the shared status-register RMW
        # sequences between the IRQ daemon thread (handleInterrupt) and the
        # main ranging loop. Reentrant so nested locked calls (e.g. startReceive
        # -> writeBytes) don't self-deadlock. Set before begin() arms the IRQ.
        self._spi_lock = threading.RLock()
        # Diagnostic counters, incremented in handleInterrupt where SYS_STATUS is
        # freshest. RX-error bits (margin) vs timeout bits (no signal) vs good
        # frames let us tell a link-margin problem from a host-timing one.
        self.irq_counts = {
            "good": 0, "tx": 0,
            "LDEERR": 0, "RXFCE": 0, "RXPHE": 0, "RXRFSL": 0,   # receiveFailed
            "RXRFTO": 0, "RXPTO": 0, "RXSFDTO": 0,              # receiveTimeout
        }
    ...
    def begin(self, irq, rst=None, bus=None, device=None):
        ...
        self.spi.mode = 0                  # DW1000 = SPI mode 0 (CPOL0/CPHA0)
        self.spi.max_speed_hz = 2000000    # <3 MHz is required until PLL/XTAL up
    ...
    def setup(self, ss):
        ...
        """
        # CSn is wired to GPIO8/CE0 and driven in hardware by spidev; `ss` is
        # retained only for API compatibility (no manual chip-select GPIO).
        self._chipSelect = ss
    ...
    def handleInterrupt(self, channel):
        """
        Callback invoked on the rising edge of the interrupt pin. Handle the
        configured interruptions.

        Held under the SPI lock for the whole service (codex High 1: the IRQ
        daemon thread must not interleave SPI / shared-register RMW with the
        main ranging loop). Instead of one SYS_STATUS snapshot followed by a
        blanket clearAllStatus() — which would wipe any cause that latched
        between the read and the clear without ever servicing it — we re-read
        SYS_STATUS in a bounded loop, servicing and clearing ONLY the bits we
        saw set, until no enabled cause remains (codex High 3).
        """
        with self._spi_lock:
            for _ in range(C.IRQ_DRAIN_MAX):
                ...
                receiveFailed = self.isReceiveFailed()
                receiveTimeout = self.isReceiveTimeout()

                if not (transmitDone or receiveTimeStampAvailable or receiveFailed
                        or receiveTimeout or msgReceived):
                    break   # no serviceable cause left — drained

                if transmitDone:
                    self.irq_counts["tx"] += 1
                    self.callbacks["handleSent"]()
                    self.clearTransmitStatus()
                    # High-5 fix: re-arm RX now that the frame has actually
                    # departed (TXFRS) — never during a pending/delayed TX. Skip
                    # if an RX cause is also pending this pass; its own branch
                    # below re-arms, and idling/clearing here would drop it.
                    # Also skip when WAIT4RESP armed the receiver in hardware:
                    # re-arming here would TRXOFF the HW-enabled RX and reintroduce
                    # the host turnaround race that W4R exists to eliminate.
                    if (self._permanentReceive and not self._w4r_armed
                            and not (receiveTimeStampAvailable or receiveFailed
                                     or receiveTimeout or msgReceived)):
                        self.newReceive()
                        self.startReceive()
                if receiveTimeStampAvailable:
                    # clear ONLY LDEDONE (write-1-to-clear zeroed mask)
                    ldeMask = [0] * 5
                    self.setBit(ldeMask, 5, C.LDEDONE_BIT, True)
                    self.writeBytes(C.SYS_STATUS, C.NO_SUB, ldeMask, 5)
                if receiveFailed:
                    for b in ("LDEERR", "RXFCE", "RXPHE", "RXRFSL"):
                        if self.getBit(self._sysstatus, 5, getattr(C, b + "_BIT")):
                            self.irq_counts[b] += 1
                    ...
                    self.irq_counts["good"] += 1
                    self.callbacks["handleReceived"]()
                    self.clearReceiveStatus()
                    if self._permanentReceive:
                        # permanent receive (RXAUTR) re-arms the receiver in HW;
                        # a newReceive() here interfered with reception.
                        self.startReceive()
    ...
    def enableMode(self, mode):
        ...
        # setChannel — default ch5, overridable via UWB_CHANNEL for bench A/B
        # (e.g. UWB_CHANNEL=2 moves off the 6 GHz band shared with WiFi 6E/7).
        self.setChannel(int(os.environ.get("UWB_CHANNEL", C.CHANNEL_5)))
    ...
    def loadLDOTUNE(self):
        """Load the per-chip RF-LDO voltage calibration (LDOTUNE) from OTP.

        The canonical Decawave init and the rust-dw1000 crate read OTP word 0x04
        (low) and, if non-zero, OTP word 0x05 (high), and write the combined
        40-bit value to the LDOTUNE register (RF_CONF 0x28:0x30). Our original
        port skipped this entirely, leaving the chip's internal RF regulators at
        reset defaults. See hl.rs init() in the dw1000 crate.
        """
        low = self.readBytesOTP(C.LDOTUNE_OTP_LOW, [0] * 4)
        # Decawave gates on (LDOTUNE_low & 0xFF) != 0; honour the low word here.
        if low[0] or low[1] or low[2] or low[3]:
            high = self.readBytesOTP(C.LDOTUNE_OTP_HIGH, [0] * 4)
            ldotune = [low[0], low[1], low[2], low[3], high[0]]
            self.writeBytes(C.RF_CONF, C.LDOTUNE_SUB, ldotune, C.LEN_LDOTUNE)
    ...
    def clearReceiveStatus(self):
        """
        Clear ONLY the reception-related SYS_STATUS bits. SYS_STATUS is
        write-1-to-clear, so we build a zeroed mask with just the RX bits set
        and write that — writing back the read-in self._sysstatus instead would
        also clear any other cause latched at read time (e.g. a TX-done that
        hasn't been serviced yet), losing it (codex High 3).
        """
        mask = [0] * 5
        self.setBit(mask, 5, C.RXDFR_BIT, True)
        self.setBit(mask, 5, C.LDEDONE_BIT, True)
        self.setBit(mask, 5, C.LDEERR_BIT, True)
        self.setBit(mask, 5, C.RXPHE_BIT, True)
        self.setBit(mask, 5, C.RXFCE_BIT, True)
        self.setBit(mask, 5, C.RXFCG_BIT, True)
        self.setBit(mask, 5, C.RXRFSL_BIT, True)
        # RX-timeout W1C bits: the IRQ timeout branch clears via this helper, and
        # the blanket clearAllStatus() that used to catch these is gone, so they
        # must be in the mask or the IRQ would stay asserted (codex High 3).
        self.setBit(mask, 5, C.RXRFTO_BIT, True)
        self.setBit(mask, 5, C.RXPTO_BIT, True)
        self.setBit(mask, 5, C.RXSFDTO_BIT, True)

        self.writeBytes(C.SYS_STATUS, C.NO_SUB, mask, 5)
    ...
    def startTransmit(self, wait4resp=False):
        """
        ...
        Args:
                wait4resp: if True, also set WAIT4RESP so the receiver auto-enables
                    the instant this frame departs (TXFRS), entirely in hardware.
                    Use on any TX that expects an immediate reply (POLL, RANGE,
                    POLL_ACK). This removes the host TX->RX turnaround from the
                    critical path — the fix for the 6.8 Mbps modes, where the short
                    preamble of the reply flies past before a software re-arm can
                    run (measured: POLL_ACK/RANGE_REPORT lost with ZERO RX-error
                    bits — the frame arrived while RX was simply not armed).
        """
        self.writeBytes(C.TX_FCTRL, C.NO_SUB, self._txfctrl, 5)
        self.setBit(self._sysctrl, 4, C.SFCST_BIT, False)
        if wait4resp:
            self.setBit(self._sysctrl, 4, C.WAIT4RESP_BIT, True)
            self._w4r_armed = True
        else:
            self._w4r_armed = False
        self.setBit(self._sysctrl, 4, C.TXSTRT_BIT, True)
        self.writeBytes(C.SYS_CTRL, C.NO_SUB, self._sysctrl, 4)
        # Do NOT arm RX here in software. For a delayed TX the frame has not
        # departed yet (DX_TIME is in the future), so enabling RXENAB now overlaps
        # the pending transmit on this half-duplex radio. When wait4resp was set,
        # the hardware arms RX at TXFRS; otherwise the TXFRS branch of
        # handleInterrupt re-arms once the frame is sent (RANGE_REPORT, no reply).
        self._deviceMode = C.IDLE_MODE

    def clearTransmitStatus(self):
        """
        Clear ONLY the transmission-related SYS_STATUS bits, via a zeroed
        write-1-to-clear mask (see clearReceiveStatus — avoids clobbering an
        unserviced RX cause latched at read time; codex High 3).
        """
        mask = [0] * 5
        self.setBit(mask, 5, C.TXFRB_BIT, True)
        self.setBit(mask, 5, C.TXPRS_BIT, True)
        self.setBit(mask, 5, C.TXPHS_BIT, True)
        self.setBit(mask, 5, C.TXFRS_BIT, True)
    ...
    def readBytes(self, cmd, offset, data, n):
        ...
        # One batched transaction: hardware CE0 stays asserted for the whole
        # frame (header then n JUNK bytes clocked out to read n bytes back).
        tx = [int(header[i]) for i in range(headerLen)] + [C.JUNK] * n
        with self._spi_lock:
            resp = self.spi.xfer2(tx)
        for i in range(0, n):
            data[i] = resp[headerLen + i]
        return data
    ...
    def writeBytes(self, cmd, offset, data, dataSize):
        ...
        # One batched transaction: header then the payload bytes (skipping any
        # None entries, as the original did), CE0 asserted for the whole frame.
        tx = [int(header[i]) for i in range(headerLen)]
        tx += [int(data[i]) for i in range(dataSize) if data[i] is not None]
        with self._spi_lock:
            self.spi.xfer2(tx)
    ...
    def setBit(self, data, n, bit, val):
        ...
        """
        idx = bit // 8     # py3: integer index (was py2 int division)
        if idx >= n:
            return
        shift = bit % 8
        if val:
    ...
    def getBit(self, data, n, pos):
        ...
        """
        idx = pos // 8     # py3: integer index (was py2 int division)
        if idx >= n:
            return
        shift = pos % 8
        # Non-mutating read. The upstream version assigned the extracted bit
        # back into data[idx], corrupting the byte for any later getBit on the
        # same byte — masked before only because handleInterrupt's trailing
        # blanket clearAllStatus() overwrote the array anyway (codex High 2).
        return (data[idx] >> shift) & 0x01
